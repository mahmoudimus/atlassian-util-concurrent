...
/**
 * Thread-safe (see note) reference that is not constructed until required. This class is used to
 * maintain a reference to an object that is expensive to create and must be constructed once and
 * once only. Therefore this reference behaves as though the <code>final</code> keyword has been
 * used (you cannot reset it once it has been constructed).
 * <p>
 * When using this class you need to implement the {@link #create()} method to return the object
 * this reference will hold.
 * <p>
 * For instance:
 * <p>
 * 
 ...
 * to create the value, instead of calling {@link #get()} call {@link #getInterruptibly()}. If your
 * {@link #create()} method throws an {@link InterruptedException} however, it is treated as an
 * application exception and will be the causal exception inside the runtime
...
public abstract class LazyReference<T> extends WeakReference<T> {
     ...
     *             {@link InitializationException#getCause()} will contain the exception thrown by
     ...
     * throws an Exception, calls to <code>get()</code> will throw a RuntimeException which wraps
     ...
     *             thread to create the value (if the creating thread is interrupted while blocking
     *             on something, the {@link InterruptedException} will be thrown as the causal
     *             exception of the {@link InitializationException} to everybody calling this
     *             method).
     */
