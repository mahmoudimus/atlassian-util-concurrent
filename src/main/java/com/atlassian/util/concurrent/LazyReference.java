...
/**
 * Thread-safe (see note) reference that is not constructed until required. This
 * class is used to maintain a reference to an object that is expensive to
 * create and must be constructed once and once only. Therefore this reference
 * behaves as though the <code>final</code> keyword has been used (you cannot
 * reset it once it has been constructed).
 * <p>
 ...
 * call {@link #getInterruptibly()}. If your {@link #create()} method throws an
 * {@link InterruptedException} however, it is treated as an application
 * exception and will be the causal exception inside the runtime
