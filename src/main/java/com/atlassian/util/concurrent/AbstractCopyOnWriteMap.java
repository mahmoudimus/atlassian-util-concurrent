 ...
 * navigable maps.
 */
@ThreadSafe abstract class AbstractCopyOnWriteMap<K, V, M extends Map<K, V>> implements ConcurrentMap<K, V>, Serializable {
   ...
   */
  @GuardedBy("lock") abstract <N extends Map<? extends K, ? extends V>> M copy(N map);
  ...
  public static abstract class View<K, V> {
    ...
    public enum Type {
      STABLE {
        @Override <K, V, M extends Map<K, V>> View<K, V> get(final AbstractCopyOnWriteMap<K, V, M> host) {
